...
BASE_CMD="python run_ppl_load_test.py --host $HOST --port $PORT --ssl --username $USERNAME --password $PASSWORD --profile single_group_power2 --duration $DURATION --ramp-step $RAMP_STEP --index $INDEX"

echo "Starting PPL Target Query Load Tests..."
...
echo "=== Test 2: default query ==="
echo "Command: $BASE_CMD --target-query default"
$BASE_CMD --target-query default
echo ""
...
# Test 4: date_histogram_hourly_agg query (if available)
...
# Test 5: keyword_terms query (if available)
echo "=== Test 5: keyword_terms query ==="
echo "Command: $BASE_CMD --target-query keyword_terms"
$BASE_CMD --target-query keyword_terms
echo ""

echo "All target query tests completed!"